...
# Fitness en CPU
def cpu_function():
    return problem.fitness(solutions)

cpu_times = timeit.repeat(cpu_function, number=1, repeat=N)
print("Fitness CPU:", problem.fitness(solutions)[0])
print(f"Time (CPU): {np.mean(cpu_times):.6f}s ± {np.std(cpu_times):.6f}s")
...
# Fitness con OpenMP
def omp_function():
    return problem.fitness_omp(solutions)

omp_times = timeit.repeat(omp_function, number=1, repeat=N)
print("Fitness OpenMP:", problem.fitness_omp(solutions)[0])
print(f"Time (OpenMP): {np.mean(omp_times):.6f}s ± {np.std(omp_times):.6f}s")
...
# Fitness en GPU
def gpu_function():
    return problem.fitness_gpu(solutions)

utils_gpu.warmup()
gpu_times = timeit.repeat(gpu_function, number=1, repeat=N)
print("Fitness GPU:", problem.fitness_gpu(solutions)[0])
print(f"Time (GPU): {np.mean(gpu_times):.6f}s ± {np.std(gpu_times):.6f}s")
